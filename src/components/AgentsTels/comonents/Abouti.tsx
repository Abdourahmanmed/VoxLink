...
export default function AboutiState() {
    //state 
    ...
    const fetchAbouti = async (data: z.infer<typeof SelectionCompagne>) => {
        const apiUrl = `http://192.168.100.4:8080/Vox_Backend//api.php?method=AfficherAppelAbouti&id=${session?.user?.id}`;
        try {
            const playload = {
                Compagne: data
            }
    ...
    return (
        <div className="flex gap-4">
            <div className="bg-white w-[50%] h-max rounded p-4 shadow-blue">
                <h1 className="text-center capitalize p-4 text-blue font-semibold">Liste de contact Abouti</h1>
                <Form {...selcte}>
                    <FormField
